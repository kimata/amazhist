...
def get_category_list(item_list):
    ...
    category_list = filter(lambda category: (float(category_map[category]) / len(item_list)) > 0.01, category_map.keys())

    # NOTE: プロットしたときに見やすくなるように，総量が多い順でソート
    return sorted(category_list, key=lambda category: category_map[category], reverse=True)
...
def get_subcategory_list(item_list, category):
    ...
    # NOTE: 数量が全体に締める割合が 1% 未満のカテゴリは捨てる
    subcategory_list = filter(lambda subcategory: (float(subcategory_map[subcategory]) / len(item_list)) > 0.01, subcategory_map.keys())

    # NOTE: プロットしたときに見やすくなるように，総量が多い順でソート
    return sorted(subcategory_list, key=lambda subcategory: subcategory_map[subcategory], reverse=True)
...
def create_graph_impl(file_path, title, xpos, ypos, zpos, dx, dy, dz,
                      xticks, xlabel, ylabel, zlabel,
                      func=None):
    ...
    font_bold = matplotlib.font_manager.FontProperties(fname=FONT_BOLD_FILE, weight='bold')
...
def create_by_year_price_hist_graph(file_path, item_list, year_list):
    ...
    def custom_format(plt, ax, font):
        ...
        plt.xticks(range(int(PRICE_HIST_MAX/PRICE_HIST_BIN*2), PRICE_HIST_MAX+int(PRICE_HIST_MAX/PRICE_HIST_BIN*2), int(PRICE_HIST_MAX/PRICE_HIST_BIN*2)),
                   ha='left', fontproperties=font)
    
    create_graph_impl(
        file_path, 'Amazon での購入価格ヒストグラムの時系列変化',
...
def create_by_year_category_count_graph(file_path, item_list, year_list, category_list):
    by_year_category_count = get_by_year_category_count(item_list, year_list, category_list)
    ...
    dz = np.array([ [ by_year_category_count[year][category] for category in category_list] for year in year_list ]).flatten()

    def custom_format(plt, ax, font):
        plt.xticks(range(0, len(category_list)), category_list,
                   ha='left', rotation=-40, fontproperties=font)

    create_graph_impl(
        file_path, 'Amazon での購入カテゴリの時系列変化',
        xpos, ypos, zpos, dx, dy, dz,
        category_list,
        None, '年', '数量',
        custom_format
...
def create_by_year_subcategory_count_graph(file_path, item_list, year_list, category):
    ...
    by_year_subcategory_count = get_by_year_subcategory_count(item_list, category, year_list, subcategory_list )
    ...
    dz = np.array([ [ by_year_subcategory_count[year][subcategory] for subcategory in subcategory_list] for year in year_list ]).flatten()

    def format_xaxis(plt, ax, font_prop):
        plt.xticks(range(0, len(subcategory_list)), subcategory_list,
                   ha='left', rotation=-40, fontproperties=font_prop)

    create_graph_impl(
        file_path, 'Amazon での購入カテゴリ({0:s})の時系列変化'.format(category),
        xpos, ypos, zpos, dx, dy, dz,
        subcategory_list,
        None, '年', '数量',
        format_xaxis
...
def create_graph(json_path):
    ...
    create_by_year_price_hist_graph(
        '時系列変化_購入価格.png', item_list, year_list
    )
    ...
    for i in category_list[0:SUBCATEGORY_NUM]:
        create_by_year_subcategory_count_graph(
            '時系列変化_購入サブカテゴリ({0:s})_数量.png'.format(i),
            item_list, year_list, i
        )
        print('.', end='', flush=True)
    print('')
    print('完了しました.')
