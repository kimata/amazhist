...
実行して良いか聞かれるので問題無ければ ENTER を押します．

最初にツールを使う際は，画像認証とSMSによる認証が必要になります．メッ
セージに従って，captcha.png に書かれた文字列を入力し，スマホにきたログ
イン許可を求めるメッセージに応答するとログインできるようになります．画
像認証への対応は複数回必要になると思います．

`amazhist.exe` の動作完了後，`amazexcel.exe` を実行します．
...
実行ファイルの末尾を .rb に置き換えたファイルを参照願います．
実行ファイルは，OCRA を使って，Ruby スクリプトから生成しています．
